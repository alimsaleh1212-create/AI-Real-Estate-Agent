...
def client() -> Generator[TestClient, None, None]:
    ...
    with (
        ...
        patch("app.main.get_stats", return_value=mock_stats),
    ):
...
class TestPredictEndpoint:
    ...
    def test_success_returns_200(self, client: TestClient) -> None:
        with (
            patch("app.main.extract_features", return_value=self._full_extraction()),
            patch("app.main.predict_price", return_value=220000.0),
            patch("app.main.predict_and_interpret", return_value="Nice house."),
        ):
    ...
    def test_extraction_failure_returns_error_field(self, client: TestClient) -> None:
        ...
        with patch("app.main.extract_features", side_effect=err):
    ...
    def test_prediction_failure_returns_error_field(self, client: TestClient) -> None:
        with (
            patch("app.main.extract_features", return_value=self._full_extraction()),
            patch("app.main.predict_price", side_effect=RuntimeError("model error")),
        ):
    ...
    def test_interpretation_failure_still_returns_price(
        ...
        with (
            patch("app.main.extract_features", return_value=self._full_extraction()),
            patch("app.main.predict_price", return_value=180000.0),
            patch(
                "app.main.predict_and_interpret",
    ...
    def test_unexpected_exception_returns_error_field(
        ...
        with patch("app.main.extract_features", side_effect=Exception("unexpected")):
            resp = client.post("/predict", json={"query": "3 bed house"})
        data = resp.json()
        assert data["error"] is not None
...
class TestInsightsEndpoint:
    def test_analysis_intent_returns_answer(self, client: TestClient) -> None:
        with (
            patch("app.main.classify_intent", return_value="analysis"),
            patch(
                "app.main.generate_market_insights",
    ...
    def test_prediction_intent_redirects(self, client: TestClient) -> None:
        with patch("app.main.classify_intent", return_value="prediction"):
